...
export default function ERC1155Deposit({ dappAddress, fetch }: CommonProps) {
    ...
    async function loadBalances(batch: { tokenId: string; value: number; balance?: number }[]) {
        const bytecode = signer.provider.getCode(erc1155address)
        if (bytecode === '0x') {
    ...
    return (
        <div>
            <h2 className="mt-8 mb-2 text-2xl text-center">ERC-1155</h2>
        <div className="flex justify-evenly">
            <div>
            <h3 className="text-lg">Single Deposit</h3>
            <span> Token address:</span> <input className="border rounded p-2 my-2  text-black" type="number"  value={erc1155address} onChange={(e) => setErc1155Address(e.target.value)} /><br />
            <span>Token ID:</span>  <input className="border rounded p-2 my-2  text-black" type="number"  value={tokenId} onChange={(e) => setTokenId(e.target.value)} /><br />
            <span>Value:</span>  <input className="border rounded p-2 my-2  text-black" type="number"  value={value} onChange={(e) => setValue(+e.target.value)} /><br />
            <Button className="bg-blue-500 p-2 rounded m-2" onClick={singleDeposit}>Deposit</Button>
            </div>      
            <div>
            <h3 className="text-lg my-4">Batch Deposit</h3>
           <span> Token address: </span> <input className="border rounded p-2 my-2  text-black" value={erc1155address} onChange={(e) => setErc1155Address(e.target.value)} />
           ...
           <span> Batch size:</span> <input className="border rounded p-2 my-2  text-black" type="number"  value={batchSize} onChange={(e) => {
                updateBatchSize(+e.target.value)
            }} /><br />
            {batch.map((item, i) => {
                return (
                    <div key={`${i}`} >
                        Token ID: <input className="border rounded p-2 my-2  text-black" type="number"  value={item.tokenId} onChange={(e) => {
                            batch[i].tokenId = e.target.value;
                            loadBalances([...batch])
                        }} /><br />
                        Value: <input className="border rounded p-2 my-2  text-black" type="number"  value={item.value} onChange={(e) => {
            ...
            <Button className="bg-blue-500 p-2 rounded m-2" onClick={batchDeposit}>Deposit</Button>{" "}
            <Button className="bg-blue-500 p-2 rounded m-2" onClick={batchWithdraw}>Voucher Withdraw</Button><br />
            <input className="border rounded p-2 my-2  text-black" type="number"  value={toAddress} onChange={(e) => {
                setToAddress(e.target.value)
            }} />
            <Button className="bg-blue-500 p-2 rounded m-2" onClick={transferErc1155}>L2 Transfer</Button>
            </div>
            </div>
        </div>
    )
